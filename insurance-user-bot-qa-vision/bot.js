...
class GenaBot {
     ...
     */
    constructor(qnaSettings, conversationState, userState) {
        ...
        this.dialogs.add(new TopMenu(this.qnaSettings, conversationState, this.userProfileAccessor, this.conversationAccessor));
    }
    ...
    async onTurn(turnContext) {
        ...
        if (turnContext.activity.type === ActivityTypes.Message) {
            ...
            if (turnContext.activity.from.name !== 'User' && turnContext.activity.from.name !== 'You')
            {
                ...
                userProfile.userRole = turnContext.activity.from.userRole;
            }
        ...
        // If Genex should ever switch to a Teams, Skype, or Slack type channel, then this logic will need to be modified.
            ...
            if (turnContext.activity && turnContext.activity.membersAdded) {
                ...
                if (conversationMembers[0].id === turnContext.activity.recipient.id) {
                    ...
                    const actv = TurnContext.applyConversationReference({ type: ActivityTypes.Event, name: "init" }, reference);
                    await dc.context.sendActivities([actv]);
                    // console.log("sent the activity reply");

                    await dc.context.sendActivity(`Hi! I’m Genna, the Social Security bot.`);
                }
            }
        }
        else {
...
module.exports.GenaBot = GenaBot;